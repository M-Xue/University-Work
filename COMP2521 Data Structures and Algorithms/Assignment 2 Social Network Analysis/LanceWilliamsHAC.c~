...
// Max Xue z5267325
// Implementation of Hierarchical Agglomerative Clustering (HAC) algorithm 
// to discover communities in a given graph using the Lance-Williams algorithm.
...
static Dendrogram createLeafCluster(Vertex v);
static Dendrogram mergeCluster(Dendrogram d1, Dendrogram d2);
static double **updateClusterDistanceArray(int numClusters, 
double **distancesArray, int mergedCluster1, int mergedCluster2, int method, 
Graph g);
...
Dendrogram LanceWilliamsHAC(Graph g, int method) {
    ...
    while (numClusters > 1) {
        ...
        currR, currC, method, g);
    }
    
    // freeing the distances array
    for (int i = 0; i < numClusters; i++) {
        free(distances[i]);
    }
    free(distances);
...
// This function creates a cluster out of a single vertex.
static Dendrogram createLeafCluster(Vertex v) {
...
static Dendrogram mergeCluster(Dendrogram d1, Dendrogram d2) {
    Dendrogram newDendrogram = malloc(sizeof(Dendrogram));
    newDendrogram->vertex = -1; // Helps check if dendrogram is leaf. If v == -1
                                // then its not a leaf.
...
// This function returns a new distance array, copying all index elements of 
// the original distance array given to it except the coloumns and rows being
// merged. It then adds the new cluster to the end column and row and updates 
// all relevant distances to the new cluster.
static double **updateClusterDistanceArray(int numClusters, 
double **distancesArray, int mergedCluster1, int mergedCluster2, int method, 
Graph g) {
    
    // Changes the Lance-Williams Formula depending on the given method 
    float a = 0.5;
    float y;
    switch (method) {
        case SINGLE_LINKAGE:
    ...
    for (int i = 0; i < numClusters + 1; i++) {
        for (int j = 0; j < numClusters + 1; j++) {
            ...
            i != mergedCluster1 && i != mergedCluster2) {
    ...
    // Updating the distances of the new cluster and old clusters
    int j = 0;
    for (int i = 0; i < numClusters; i++) {
        ...
        if (i != mergedCluster1 && i != mergedCluster2) {
            ...
            float absDifference = distancesArray[i][mergedCluster1] - 
            distancesArray[i][mergedCluster2];
            if (absDifference < 0) {
                absDifference = -absDifference;
            }
            
            float LanceWilliamsDistance = a * 
            distancesArray[i][mergedCluster1] + 
    ...
    // freeing the original distances array
    for (int i = 0; i < numClusters; i++) {
