...
printf("Test edgeWeight = %d\n\n", edgeWeight);

printf("\nTest sps.dist[currExploredVertexChild->v] = %d\n\n", sps.dist[currExploredVertexChild->v]);
*/
...
Old extra predecessors feature 
/*
if (sps.dist[minDistIndex] + edgeWeight == 
sps.dist[currExploredVertexChild->v]) {
    ...
    while (currVertexPredListNode->next != NULL) {
        currVertexPredListNode = currVertexPredListNode->next;
    }
    // Should stop at the last node so currVertexPredListNode->next
    // points at NULL at this point 
    
    currVertexPredListNode->next = malloc(sizeof(PredNode));
    currVertexPredListNode->next->next = NULL;
    currVertexPredListNode->next->v = minDistIndex;
}
*/
